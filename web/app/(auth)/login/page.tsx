...
export default function HalamanLogin() {
    ...
    // Status formulir masuk dengan penamaan bahasa Indonesia
    const [formulirMasuk, setFormulirMasuk] = useState({ email: '', kataSandi: '' });
    const [pesanKesalahan, setPesanKesalahan] = useState<string>('');
    const [sedangMemuat, setSedangMemuat] = useState<boolean>(false);
    
    // Status visual tambahan untuk kata sandi, mengingat sesi, dan Google login
    const [tampilkanKataSandi, setTampilkanKataSandi] = useState<boolean>(false);
    const [ingatSaya, setIngatSaya] = useState<boolean>(false);
    const [pesanGoogle, setPesanGoogle] = useState<string>('');

    // Menangani aksi masuk log menggunakan email dan kata sandi
    async function menanganiMasuk(peristiwa: React.FormEvent) {
                ...
                <div className="bg-white rounded-2xl shadow-xl shadow-slate-100/50 border border-slate-100 p-8 space-y-6 relative">
                    ...
                    <div className="text-center space-y-2">
                        ...
                        <p className="text-xs sm:text-sm text-slate-500">
                            Selamat datang kembali di komunitas Anda.
                        </p>
                    </div>
                    ...
                    {/* Menampilkan pesan simulasi Google Login */}
                    {pesanGoogle && (
                    ...
                    {/* Formulir Masuk Utama */}
                    <form onSubmit={menanganiMasuk} className="space-y-4">
                        {/* Input Alamat Email */}
                        ...
                        <div className="flex items-center justify-between text-xs pt-1">
                            <label className="flex items-center gap-2 text-slate-600 font-medium cursor-pointer">
                                <input 
                                    ...
                                    className="rounded border-slate-300 text-[#0F3A4B] focus:ring-[#0F3A4B]/20 w-4 h-4 cursor-pointer" 
                                />
                                <span>Ingat Saya</span>
                            </label>
                            <Link href="#" className="text-[#0F3A4B] hover:underline font-semibold" onClick={() => alert('Fitur pemulihan kata sandi sedang dalam tahap pengembangan.')}>
                                Lupa Kata Sandi?
                            </Link>
                        </div>
                        ...
                        <button 
                            ...
                            className="w-full bg-[#0F3A4B] text-white py-3 rounded-xl font-bold text-xs sm:text-sm shadow-md shadow-[#0F3A4B]/10 hover:bg-[#0c2e3c] transition active:scale-[0.99] disabled:opacity-60 cursor-pointer flex items-center justify-center gap-2"
                        >
                            {sedangMemuat ? (
                                <>
                    ...
                    {/* Bagian Bawah: Daftar Akun Baru */}
                    <p className="text-center text-xs sm:text-sm text-slate-500 border-t border-slate-100 pt-4">
                        Belum punya akun? <Link href="/signup" className="text-[#0F3A4B] font-bold hover:underline">Daftar</Link>
                    </p>
                </div>
            </div>
        </div>
